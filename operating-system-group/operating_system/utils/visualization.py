...
class Visualizer:
    ...
    def draw_gantt_chart(self, gantt_data: List[GanttEntry], algorithm_name: str, 
        ...
        """
        if not gantt_data:
            print(f"No Gantt chart data for {algorithm_name}")
            return
    ...
    def print_statistics_table(self, results: List[Dict]):
        ...
        """
        print("\n" + "="*120)
        print("SCHEDULING ALGORITHMS PERFORMANCE COMPARISON")
        print("="*120)
        print(f"{'Algorithm':<30} {'Avg WT':>12} {'Avg TAT':>12} {'CPU Util(%)':>15} {'Context SW':>12}")
        print("-"*120)
    ...
    def print_process_details(self, results: Dict):
        ...
        """
        print(f"\n{'='*80}")
        print(f"Process Details - {results['algorithm']}")
        print(f"{'='*80}")
        print(f"{'PID':<6} {'Arrival':>8} {'Priority':>10} {'Start':>8} {'Finish':>8} "
